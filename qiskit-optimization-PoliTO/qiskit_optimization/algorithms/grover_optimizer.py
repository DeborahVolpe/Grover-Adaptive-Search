...
"""Code integrated by L. Giuffrida, D.Volpe, G.A. Cirillo, M.Zamboni and G.Turvani"""
"""for supporting the stategies for degrees of freedom management presented in the"""
"""article 'Engineering Grover Adaptive Search: Exploring the Degrees of Freedom for Efficient QUBO Solving'"""
"""published on 'IEEE JOURNAL ON EMERGING AND SELECTED TOPICS IN CIRCUITS AND SYSTEMS' in 30/09/2022"""
...
handler = logging.handlers.WatchedFileHandler(
    os.environ.get("LOGFILE", "prova.log"))
formatter = logging.Formatter(logging.BASIC_FORMAT)
handler.setFormatter(formatter)
logger = logging.getLogger(__name__)

#########################! added logile as parameter and changed logger.info with logfile.write ###################################
...
class GroverOptimizer(OptimizationAlgorithm):
    ...
    def solve(self, problem: QuadraticProgram) -> OptimizationResult:
        ...
        while not optimum_found:
            ...
            while not improvement_found:
                classic_iterations += 1
                # Determine the number of rotations.
                loops_with_no_improvement += 1
                rotation_count = algorithm_globals.random.integers(0, m)
                self.logfile.write(f"Rotation count: {rotation_count}\n")
                ...
                stop = time.time()
                GroverExecutionTime = stop-start
                self.logfile.write(f"Grover Execution Time {GroverExecutionTime}\n")
                k = int(outcome[0:n_key], 2)
                v = outcome[n_key : n_key + n_value]
                int_v = self._bin_to_int(v, n_value) + threshold
                self.logfile.write(f"Outcome: {outcome}\n")
                self.logfile.write(f"Value Q(x): {int_v}\n")
                # If the value is an improvement, we update the iteration parameters (e.g. oracle).
                if int_v < optimum_value:
                    optimum_key = k
                    optimum_value = int_v
                    self.logfile.write(f"Current Optimum Key: {optimum_key}\n")
                    self.logfile.write(f"Current Optimum Value: {optimum_value}\n")
                    ...
                    self.logfile.write(f"No Improvement. M: {m}\n")
                ...
                iteration += 1
                self.logfile.write(f"Operation Count: {operations}\n")
        ...
        self.logfile.write(f"Classical iterations: {classic_iterations}\n")
    ...
    def solve_PoliTo_adaptT(self, problem: QuadraticProgram, adaptTlin, adaptTlog, tmin_par, vel_par, restore = True, update = True, random = True, pattern = False, linear = False, updateonimprove = False, satM = False, adaptT = False) -> OptimizationResult:
        ...
        Args:
            ...
            Flags for choosing the threshold management strategy:
                adaptTlin: flag which is equal to True if the linear stategy for managing the threshold for the stop condition
                adaptTlog: flag which is equal to True if the logarithmic stategy for managing the threshold for the stop condition
            ...
            Flags for choosing the number of grover rotation management strategy:
                ...
                update: flag which is equal to True for random/linear stategy always and for pattern strategy if a new value is read at every grover search execution
                random: flag which is equal to True if the random strategy for managing the number of grover rotation
                pattern: flag which is equal to True if the fixed pattern strategy for managing the number of grover rotation
                linear: flag which is equal to True if the linear strategy for managing the number of grover rotation
                updateonimprove: flag which is equal to True if a new value of the fixed pattern is read only when a negative sample is obtained
                satM: flag which is equal to True if the interval considered for random/linear strategy is saturated 
        ...
        """ 
            self._n_iterations is the starting value of the threshold, so it is equal to the maximum threshold
            for the linear and logarithmic strategie for the stop condition management and to the initial value for
            the adaptive variation strategy
        """
        
        
        #if the adaptive dynamic variation of the threshold is condered
        if adaptT:
            posCNT = 0
            negCNT = 0
        ...
        classic_iterations = 0
        if pattern:
            # pointer to the fixed random pattern list
            m = 0
            # fixed random pattern
            PAT = [0, 0, 0, 1, 1, 0, 1, 1, 2, 1, 2, 3, 1, 4, 5, 1, 6, 2, 7, 9, 11, 13, 16, 5, 20]
        elif random:
            # initial value for the random strategy interval extreme
            m = 1
        elif linear:
            # initial value number of Grover rotation for the linear strategy
            m = 0
        ...
        if adaptTlin or adaptTlog:
            ...
            tMin = tmin_par
            # initialization to zero of the counter of executed classical iteration
            nTrasl = 0
        
        # Until the stop condition is not met
        while not optimum_found:
            # if the restore flag is equal to True and the considered strategy is the random one
            if restore:
                # the random strategy interval extreme is again set at 1
                m = 1
            # if the restore flag is equal to True and the considered strategy is fixed pattern or linear
            if restore and (pattern or linear):
                # pointer to the fixed random pattern list or the value number of Grover rotation for the linear strategy are again set at 0
                m = 0
                
            # initialization of the improvement flag at False
            improvement_found = False
            ...
            while not improvement_found:
                ...
                classic_iterations += 1
                loops_with_no_improvement += 1
                if (satM and (random or linear)):
                    m = int(np.ceil(min(m, 2 ** (n_key / 2)))) #! SAT
                if random:
                    rotation_count = algorithm_globals.random.integers(0, m)
                if pattern:
                    rotation_count = PAT[m]
                if linear:
                    rotation_count = m
                self.logfile.write(f"Rotation count: {rotation_count}\n")
                rotations += rotation_count
                ...
                if int_v < optimum_value:
                    ...
                    if adaptTlin:
                        # evaluation of the next threshold value according to the linear shape
                        nnit = int(np.ceil((-float(tMax)/tMin)*(nTrasl/vel_par)+tMax))
                        # saturation of the next threshold value if it is lower then the minimum possible 
                        self._n_iterations = nnit if nnit > tMin else tMin
                        
                    # Update of the threshold for the logarithmic strategy
                    if adaptTlog:
                        nnit = int(np.ceil((np.log10(1.1)*(float(tMax)-tMin))/np.log10(1.1+nTrasl/vel_par) + (tMin-1)))
                        # saturation of the next threshold value if it is lower then the minimum possible 
                        self._n_iterations = nnit if nnit > tMin else tMin
                    ...
                    # for linear and logarithmic dynamic scaling of the threashold update the counter of the classical iterations
                    if adaptTlin or adaptTlog:
                        nTrasl += 1
                    ...
                    # for the fixed pattern strategy for the number of Grover rotation, if we choose to update 
                    # the list point at every improvement
                    if updateonimprove and pattern:
                        m = m + 1

                # A positive sample is obtained                    
                else:
                    # Considering Durr and Hoyer method for the fixed pattern update the list pointer
                    # For the random strategy for the number of Grover rotation update the extreme of the interval
                    # For the linear strategy update the number of Grover Rotation
                    if update and not(updateonimprove):
                        m = m + 1
                    self.logfile.write(f"No Improvement. M: {m}, t: {self._n_iterations}\n")
                    ...
                    # For the adaptive strategy for threshold dynamic variation, the number of positive sample is updated
                    if adaptT:
                        posCNT += 1
    ...
    def _measure(self, circuit: QuantumCircuit) -> str:
        ...
        self.logfile.write(f"len:{len(probs.keys())}\n")
        self.logfile.write(f"sum:{sum(probs.values())}\n")
        self.logfile.write(f"Frequencies: {probs}\n")
